...
function displayFavouritesOnStartup() {
  ...
  for (let i = 0; i < savedRecipes.length; i++) {
    ...
    const favLiEl = document.createElement(`li`)
...
function displayCarousel() {
  const randomAPI = `https://api.spoonacular.com/recipes/random?number=13&apiKey=${NatLuisLizDomAPIKey}`
...
const TrendingSlider = new Swiper(".trending-slider", {
  ...
  on: {
    slideChange: function () {
      ...
      const activeSlideTitle = imgEl[activeSlideIndex].getAttribute("data-title")
...
function buttonInit() {
  ...
  for (let i = 0; i < createdButtonsEl.length; i++) {
    createdButtonsEl[i].addEventListener(`click`, (event) => {
      // console.log(`it works`);

      const recipeId = createdButtonsEl[i].id

      const buttonIDAPI = `https://api.spoonacular.com/recipes/${recipeId}/information?apiKey=${spoonAPIKey}`
...
function createRecipeInfo(data) {
  const rName = data.title
  const rImage = data.image
...
function embedVid(data) {
  ...
  const ytLink = `https://www.youtube.com/embed/` + vidId
  // console.log(ytLink);
...
function ytAPICaller(data) {
  ...
  const apiURL = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&q=${ytQuery}&topicId=Food&key=${ytAPIKey}`
...
addIngEl.addEventListener(`click`, (event) => {
...
favButtonEl.addEventListener(`click`, (event) => {
  // console.log(displayedRecipe);
    ...
    if (selectEl.value === `By Ingredient`) {
      ...
      if (ingredientEl.length > 1) {
        for (let i = 1; i < ingredientEl.length; i++) {
          ...
          const extraIng = `,+` + ingredientEl[i].value
      ...
      const apiURL = `https://api.spoonacular.com/recipes/findByIngredients?ingredients=${spoonInput}&number=5&apiKey=${spoonAPIKey}`
      ...
      if (isVegetarianEl.checked) {
        const apiURL = `https://api.spoonacular.com/recipes/complexSearch?query=${spoonInput}&diet=vegetarian&number=5&apiKey=${spoonAPIKey}`
        clearButtons()
        spoonAPiCallerRecipe(apiURL)
      } else {
        const apiURL = `https://api.spoonacular.com/recipes/complexSearch?query=${spoonInput}&number=5&apiKey=${spoonAPIKey}`
        clearButtons()
        spoonAPiCallerRecipe(apiURL)
      }
    } else {
...
function displayFavouritesOnStartupClear() {
  ...
  for (let i = 0; i < savedRecipes.length; i++) {
    ...
    favButtonEl.setAttribute(`id`, savedRecipes[i].id)
    favButtonEl.textContent = savedRecipes[i].name
    const favDelEl = document.createElement(`button`)
...
gitHubPage.addEventListener("click", (event) => {
  window.location.href = "https://github.com/namastenataly/project-01-NLDL"
})
...
// Add these lines at the end of the Script.js file to connect with our new UI elements

// Connect the hero modal button to the search modal
document.getElementById("hero-modal-button")?.addEventListener("click", (event) => {
  openModal()
})

// Connect the CTA modal button to the search modal
document.getElementById("cta-modal-button")?.addEventListener("click", (event) => {
  openModal()
})

// Update the favorite button text and style when visible
favButtonEl.addEventListener("click", (event) => {
  if (event.target) {
    favouriteRecipe()
    favButtonEl.textContent = "Added to Favorites ❤️"
    setTimeout(() => {
      favButtonEl.textContent = "Add to Favorites"
    }, 2000)
  }
})

// Add a loading animation when fetching recipes
function showLoading() {
  const loadingEl = document.createElement("div")
  loadingEl.classList.add("loading-spinner")
  loadingEl.innerHTML = '<div class="spinner"></div><p>Finding delicious recipes...</p>'
...
// Enhance the API callers with loading indicators
const originalSpoonAPICallerIng = spoonAPICallerIng
spoonAPICallerIng = (url) => {
...
// Add this at the end of the file to ensure all images have fallbacks
document.addEventListener("DOMContentLoaded", () => {
  ...
  allImages.forEach((img) => {
    ...
    // Force load to trigger error handler if needed
